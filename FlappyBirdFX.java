...
public class FlappyBirdFX extends Application {
    ...
    private char currentLetter = 'A'; // The current letter to match
    private int gameMode = 1; // Default to 1: Spacebar Jump
    ...
    public void start(Stage stage) {
        ...
        loadImages();
        setupInput(canvas);
        generateRandomLetter(); // Generate the first letter
        if (!loggedIn) {
            showLoginPage(stage);
            return;
        }
    ...
    private void setupInput(Canvas canvas) {
        ...
        canvas.setOnKeyPressed(event -> {
            if (inMenu) return; // Ignore key events in the menu
                ...
                case 1: // Default Controls
                    if (event.getCode() == KeyCode.SPACE) {
                        if (gameOver) {
                            ...
                            velocityY = -8; // Bird jumps
                        }
                    }
                    break;

                case 2: // Letter Matching
                    if (event.getCode() == KeyCode.SPACE && gameOver) {
                        saveHighScore();
                        restartGame();
                    } else if (event.getText().equalsIgnoreCase(String.valueOf(currentLetter))) {
                        velocityY = -8; // Bird jumps on correct letter
                        generateRandomLetter(); // Refresh the letter
                    }
                    break;
                default:
                    break;
    ...
    private void handleMenuClick(double clickX, double clickY) {
        if (clickX >= BOARD_WIDTH / 2.0 - 100 && clickX <= BOARD_WIDTH / 2.0 + 100) {
            if (clickY >= BOARD_HEIGHT / 3.0 && clickY <= BOARD_HEIGHT / 3.0 + 40) {
                gameMode = 1; // Default Controls
                inMenu = false; // Start the game
            } else if (clickY >= BOARD_HEIGHT / 3.0 + 100 && clickY <= BOARD_HEIGHT / 3.0 + 140) {
                gameMode = 2; // Letter Matching
                inMenu = false; // Start the game
            } else if (clickY >= BOARD_HEIGHT / 3.0 + 160 && clickY <= BOARD_HEIGHT / 3.0 + 200) {
                gameMode = 3; // Mouse Only
                inMenu = false; // Start the game
            }
            else if (clickY >= BOARD_HEIGHT / 3.0 + 220 && clickY <= BOARD_HEIGHT / 3.0 + 260) {
                gameMode = 4; // Mouse Only
                inMenu = false; // Start the game
            }
        }
    }
...
// Display the current letter to type
    ...
    private void drawMenu(GraphicsContext gc) {
        ...
        Image backgroundImg = new Image("flappybirdbg.png");
        ...
        gc.fillText("Marvel Comicon", BOARD_WIDTH / 2.0 - 140, BOARD_HEIGHT / 6.0);

        // Buttons
        double buttonWidth = 200;
        double buttonHeight = 50;
        double buttonX = BOARD_WIDTH / 2.0 - buttonWidth / 2;

        // Button 1: Start
        drawButton(gc, buttonX, BOARD_HEIGHT / 3.0, buttonWidth, buttonHeight, "#4CAF50", "white", "Start");

        // Button 2: Letter Matching
        drawButton(gc, buttonX, BOARD_HEIGHT / 3.0 + 80, buttonWidth, buttonHeight, "#FFC107", "black", "Letter Matching");

        // Button 3: Mouse Only
        drawButton(gc, buttonX, BOARD_HEIGHT / 3.0 + 160, buttonWidth, buttonHeight, "#2196F3", "white", "Mouse Only");

        //Advance Game
        drawButton(gc, buttonX, BOARD_HEIGHT / 3.0 + 240, buttonWidth, buttonHeight, "#1ECBE1", "gray", "Iron Passer");
    }


    private void drawButton(GraphicsContext gc, double x, double y, double width, double height, String bgColor, String textColor, String text) {
        ...
        gc.fillText(text, x + width / 4.0, y + height / 1.7);
    }
    ...
    private void generateRandomLetter() {
        currentLetter = (char) ('A' + random.nextInt(26)); // Random letter A-Z
    }
    public class Pipe {
        double x, y;
        boolean passed;
